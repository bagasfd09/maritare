...
// Plum — on-the-spot guest check-in QR (new section; the Kinanti reference has
// none). Mirrors ivory-/sienna-qr's presentation: a section head ("QR Check-in"
// + guest name + explainer copy) and the QR in a framed card, dressed in plum
// identity (gold flourishes at the card corners, plum/blush palette). When the
...
export function PlumQr({ checkin }: Props) {
  ...
  useEffect(() => {
    ...
    QRCode.toDataURL(payload, {
      ...
      // Deep plum on cream — palette-tinted but still high contrast so it
      // scans fine.
      color: { dark: "#4A2C38", light: "#FFFAEB" },
    })
      .then((url) => {
        if (alive) setQr(url);
      })
      ...
      <div className="qr-inner">
        <div className="qr-head" data-aos="zoom-in" data-aos-duration="1200">
          <div className="qr-orn-header">
            <img loading="lazy" decoding="async" src={`${BASE}/orn-29-min.png`} alt="" />
          </div>
        ...
        <div className="qr-card" data-aos="fade-up" data-aos-duration="1200" data-aos-delay="200">
          {/* Plum identity — mirrored gold corner flourishes, tucked behind. */}
          <div className="qr-orn tl">
            <img loading="lazy" decoding="async" src={`${BASE}/orn-32-min.png`} alt="" />
          </div>
